...
wssClients.on('connection', (socket, req) => {
    console.log('Flutter client connected');
    clients.add(socket);

    // Prepare initial data payload combining reference and latest data
    const initialData = {
        ...referenceData,  // Include reference data
        ...latestData      // Include current state
    };
...
async function connectwss(token, cookie) {
    ...
    const p = new Promise((res, rej) => {
        ...
        // Enhance the message handler to display R messages
        sock.on('message', (data) => {
            try {
                // Parse the data
                const parsedData = JSON.parse(data);
                
                // Display R message if it exists
                if (parsedData.R) {
                    console.log('Received Reference (R) Message:');
                    // console.log(JSON.stringify(parsedData.R, null, 2));
                }
                
                // Check if this is a CarData.z message
                if (parsedData.M && Array.isArray(parsedData.M)) {
                    for (const message of parsedData.M) {
                        // Check for R message within array structure
                        if (message.H === 'Streaming' && message.M === 'feed' && 
                            message.A && Array.isArray(message.A) && message.A[0] && message.A[0].R) {
                            console.log('Received Reference (R) Message within feed:');
                            // console.log(JSON.stringify(message.A[0].R, null, 2));
                        }
                        
                        // Continue with existing CarData.z handling...
                        if (message.H === 'Streaming' && message.M === 'feed' && 
                        ...
                        if (message.H === 'Streaming' && message.M === 'feed' && message.A && Array.isArray(message.A)) {
                            const telemetryUpdate = message.A[0];
                            
                            // Store reference data with enhanced logging
                            if (telemetryUpdate && telemetryUpdate.R) {
                                referenceData.R = telemetryUpdate.R;
                                console.log('Received reference data within telemetry update:');
                                // console.log(JSON.stringify(telemetryUpdate.R, null, 2));
                                
                                // Use saveLatestDataToFile instead of direct file write
                                saveLatestDataToFile();
                            }
                            
                            // Update latest data with any new values...
                            // Existing code continues...
                            if (telemetryUpdate) {
                                Object.keys(telemetryUpdate).forEach(key => {
                                    latestData[key] = telemetryUpdate[key];
                                });
                                
                                // Save updated data to file whenever we receive updates
                                saveLatestDataToFile();
                            }
                            
                            // Broadcast normal data
                            broadcastToClients(parsedData.R);
                        }
                    }
                } else {
                    // For non-array messages (like reference data)
                    if (parsedData.R) {
                        referenceData.R = parsedData.R;
                        console.log('Received reference data (direct):');
                        // console.log(JSON.stringify(parsedData.R, null, 2));
                    ...
                    Object.keys(parsedData).forEach(key => {
                        if (key !== 'M') { // Skip message array which we process separately
                            latestData[key] = parsedData[key];
                        }
                    });
                    
                    // Save updated data after processing - keep this single call
                    saveLatestDataToFile();
...
app.get('/initialData', (req, res) => {
    // Serve the initial data JSON file
    res.sendFile(__dirname + '/initialData.json');
});

// Add this endpoint to your server setup
app.get('/carTelemetry', (req, res) => {
    if (Object.keys(carTelemetryData).length === 0) {
        res.status(404).json({
            error: 'No car telemetry data available'
        });
    } else {
        res.status(200).json(formatCarTelemetryData(carTelemetryData));
    }
});
        ...
        sock.send(JSON.stringify(
            {
                ...
                "M": "Subscribe",
                "A": [["Heartbeat",
        "CarData.z",  // Make sure this is uncommented
        // "Position.z",
        "ExtrapolatedClock",
        "TopThree",
        "RcmSeries",
